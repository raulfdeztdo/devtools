...
useSeoMeta({
  titleKey: "tools.userAgentParser.title",
...
function parseBrowser(ua) {
  ...
  if (/Edg\/(\S+)/.test(ua))
    return { name: "Microsoft Edge", version: ua.match(/Edg\/([^\s;]+)/)[1] };
  ...
  if (/SamsungBrowser\/(\S+)/.test(ua))
    return {
      ...
      version: ua.match(/SamsungBrowser\/([^\s;]+)/)[1],
    };
  if (/YaBrowser\/(\S+)/.test(ua))
    return {
      name: "Yandex Browser",
      version: ua.match(/YaBrowser\/([^\s;]+)/)[1],
    };
  if (/UCBrowser\/(\S+)/.test(ua))
    return { name: "UC Browser", version: ua.match(/UCBrowser\/([^\s;]+)/)[1] };
  if (/CriOS\/(\S+)/.test(ua))
    return { name: "Chrome (iOS)", version: ua.match(/CriOS\/([^\s;]+)/)[1] };
  if (/FxiOS\/(\S+)/.test(ua))
    return { name: "Firefox (iOS)", version: ua.match(/FxiOS\/([^\s;]+)/)[1] };
  if (/Firefox\/(\S+)/.test(ua))
    return { name: "Firefox", version: ua.match(/Firefox\/([^\s;]+)/)[1] };
  if (/MSIE\s(\S+)/.test(ua))
    return {
      name: "Internet Explorer",
      version: ua.match(/MSIE\s([^\s;]+)/)[1],
    };
  if (/Trident.*rv:(\S+)/.test(ua))
    return {
      name: "Internet Explorer",
      version: ua.match(/Trident.*rv:([^\s;)]+)/)[1],
    };
  if (/Chrome\/(\S+)/.test(ua) && !/Chromium/.test(ua))
    return { name: "Chrome", version: ua.match(/Chrome\/([^\s;]+)/)[1] };
  if (/Chromium\/(\S+)/.test(ua))
    return { name: "Chromium", version: ua.match(/Chromium\/([^\s;]+)/)[1] };
  if (/Version\/(\S+).*Safari/.test(ua))
    return { name: "Safari", version: ua.match(/Version\/([^\s;]+)/)[1] };
  if (/Safari\/(\S+)/.test(ua))
    return { name: "Safari", version: ua.match(/Safari\/([^\s;]+)/)[1] };
...
function parseEngine(ua) {
  ...
  if (/AppleWebKit\/(\S+)/.test(ua))
    return { name: "WebKit", version: ua.match(/AppleWebKit\/([^\s;]+)/)[1] };
  if (/Blink/.test(ua)) return { name: "Blink", version: "" };
  if (/Presto\/(\S+)/.test(ua))
    return { name: "Presto", version: ua.match(/Presto\/([^\s;]+)/)[1] };
  if (/Trident\/(\S+)/.test(ua))
    return { name: "Trident", version: ua.match(/Trident\/([^\s;]+)/)[1] };
  return { name: t("tools.userAgentParser.unknown"), version: "" };
}
...
function parseOS(ua) {
  ...
  if (/iPhone OS (\S+)/.test(ua))
    return {
      name: "iOS",
      version: ua.match(/iPhone OS ([^\s;)]+)/)[1].replace(/_/g, "."),
    };
  if (/iPad.*OS (\S+)/.test(ua))
    return {
      name: "iPadOS",
      version: ua.match(/OS ([^\s;)]+)/)[1].replace(/_/g, "."),
    };
  if (/Android (\S+)/.test(ua))
    return { name: "Android", version: ua.match(/Android ([^\s;)]+)/)[1] };
  if (/Mac OS X (\S+)/.test(ua))
    return {
      name: "macOS",
      version: ua.match(/Mac OS X ([^\s;)]+)/)[1].replace(/_/g, "."),
    };
  if (/CrOS/.test(ua)) {
...
function parseDeviceType(ua) {
  ...
  if (
    /Mobile|Android|iPhone|iPod|BlackBerry|IEMobile|Opera Mini|Windows Phone/.test(
      ua,
    )
  )
    return "mobile";
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600 dark:text-gray-400">
          {{ t("tools.userAgentParser.subtitle") }}
        </p>
      </div>
      ...
      <div class="p-6 mb-6 card">
        <div class="flex items-center justify-between mb-3">
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">
            {{ t("tools.userAgentParser.inputLabel") }}
          </h2>
      ...
      <div v-if="parsed" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="p-5 card dark:bg-night-card">
          ...
          <p
            ...
            class="text-sm text-brand-blue dark:text-brand-blue-lighter font-mono"
          >
      ...
      <div
        v-if="parsed"
        class="mt-4 p-5 card dark:bg-night-card dark:border-night-border"
      >
        <div class="flex items-center justify-between mb-2">
          <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">
            {{ t("tools.userAgentParser.rawString") }}
          </p>
          <button
            @click="copyRaw"
            class="btn-secondary flex items-center gap-1.5"
        ...
        <p
          class="font-mono text-xs text-gray-600 dark:text-gray-400 break-all p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner"
        >
          {{ parsed.raw }}
        </p>
      </div>
      ...
      <div v-if="!parsed && !uaInput" class="p-12 text-center card">
        ...
        <p class="text-gray-500 dark:text-gray-400">
          {{ t("tools.userAgentParser.emptyState") }}
        </p>
      </div>
        ...
        >
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.userAgentParser.infoStructureLabel") }}
            </div>
            <p class="text-xs">
              {{ t("tools.userAgentParser.infoStructure") }}
            </p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.userAgentParser.infoLimitsLabel") }}
            </div>
            <p class="text-xs">{{ t("tools.userAgentParser.infoLimits") }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.userAgentParser.infoFreezeLabel") }}
            </div>
            <p class="text-xs">{{ t("tools.userAgentParser.infoFreeze") }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.userAgentParser.infoAlternLabel") }}
            </div>
