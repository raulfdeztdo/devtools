...
const inputLabel = computed(() =>
    direction.value === 'toml-to-json' ? t('tools.tomlJson.tomlLabel') : t('tools.tomlJson.jsonLabel')
)
const outputLabel = computed(() =>
    direction.value === 'toml-to-json' ? t('tools.tomlJson.jsonLabel') : t('tools.tomlJson.tomlLabel')
)
const inputPlaceholder = computed(() =>
    direction.value === 'toml-to-json' ? t('tools.tomlJson.tomlPlaceholder') : t('tools.tomlJson.jsonPlaceholder')
)
const outputPlaceholder = computed(() =>
    direction.value === 'toml-to-json' ? t('tools.tomlJson.jsonOutputPlaceholder') : t('tools.tomlJson.tomlOutputPlaceholder')
)
...
function swapDirection() {
    ...
    error.value = ''
    direction.value = direction.value === 'toml-to-json' ? 'json-to-toml' : 'toml-to-json'
        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="mb-8 text-center">
                <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
                    <ArrowLeftRight class="inline-block w-5 h-5 mr-2" />{{ t('tools.tomlJson.title') }}
                </h1>
        ...
        <div class="p-6 mt-8 card">
            ...
            <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.tomlJson.infoWhatLabel') }}</div>
                    <p class="text-xs">{{ t('tools.tomlJson.infoWhat') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.tomlJson.infoSyntaxLabel') }}</div>
                    <p class="text-xs">{{ t('tools.tomlJson.infoSyntax') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.tomlJson.infoTypesLabel') }}</div>
                    <p class="text-xs">{{ t('tools.tomlJson.infoTypes') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.tomlJson.infoUsageLabel') }}</div>
