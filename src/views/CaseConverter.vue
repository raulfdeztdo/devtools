...
const conversions = computed(() => {
  ...
  const titled = tokens.map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
...
<template>
  <div class="py-8">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2">
            <button @click="loadSample" class="btn-secondary flex items-center gap-1.5">
              <FileText class="inline-block w-3.5 h-3.5 mr-1" />{{ t('tools.caseConverter.loadSample') }}
            </button>
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold tracking-wide uppercase text-brand-blue dark:text-brand-blue-lighter">
              {{ item.label }}
            </span>
            <button
              ...
              :title="t('common.copy')"
            >
              <CheckCircle v-if="copiedKey === item.key" class="w-4 h-4 text-green-500" />
              <ClipboardCopy v-else class="w-4 h-4" />
            </button>
          </div>
          <div class="px-3 py-2 font-mono text-sm text-gray-900 rounded-md dark:text-white bg-gray-50 dark:bg-night-card-inner break-all">
            {{ item.value || '—' }}
          </div>
        </div>
      </div>
      ...
      <!-- Empty state -->
      <div v-if="!input" class="mt-6 text-center text-sm text-gray-400 dark:text-gray-500">
      ...
      <div class="p-6 mt-8 card">
        ...
        <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.caseConverter.infoCamelLabel') }}</div>
            <p class="text-xs">{{ t('tools.caseConverter.infoCamel') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.caseConverter.infoPascalLabel') }}</div>
            <p class="text-xs">{{ t('tools.caseConverter.infoPascal') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.caseConverter.infoSnakeLabel') }}</div>
            <p class="text-xs">{{ t('tools.caseConverter.infoSnake') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.caseConverter.infoKebabLabel') }}</div>
