...
const { t } = useI18n();
useSeoMeta({
  titleKey: "tools.cssJsMinifier.title",
  descriptionKey: "tools.cssJsMinifier.description",
});
...
const sizeDiff = computed(() => {
  ...
  const pct =
    inputSizeBytes.value > 0
      ? Math.round((diff / inputSizeBytes.value) * 100)
      : 0;
  return { diff, pct };
});
...
function formatCode() {
  ...
  try {
    output.value =
      language.value === "css" ? formatCss(input.value) : formatJs(input.value);
  } catch (e) {
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="mb-3 text-2xl font-bold text-gray-900 dark:text-white">
          <Code2 class="inline-block w-6 h-6 mr-2" />{{
            t("tools.cssJsMinifier.title")
          }}
        </h1>
      ...
      <div class="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div class="flex items-center gap-2">
          <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{
            t("tools.cssJsMinifier.languageLabel")
          }}</label>
          <div
            class="flex rounded-lg overflow-hidden border border-gray-300 dark:border-night-border"
          >
            <button
              @click="
                language = 'css';
        ...
        <div class="flex gap-2">
          ...
          >
            ...
            {{ t("tools.cssJsMinifier.minifyBtn") }}
          </button>
          <button
            @click="loadSample"
            class="btn-secondary flex items-center gap-1.5"
      ...
      <div
        v-if="error"
        class="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-lg text-red-800 dark:text-red-300 text-sm"
      >
        {{ error }}
      </div>
      ...
      <!-- Size comparison -->
      <div
        v-if="output && sizeDiff"
        class="mb-4 p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner border border-gray-200 dark:border-night-border flex flex-wrap gap-4 text-sm"
      >
        <span class="text-gray-700 dark:text-gray-300">
          {{ t("tools.cssJsMinifier.inputSize") }}:
          <strong>{{ formatBytes(inputSizeBytes) }}</strong>
        </span>
        <span class="text-gray-700 dark:text-gray-300">
          {{ t("tools.cssJsMinifier.outputSize") }}:
          <strong>{{ formatBytes(outputSizeBytes) }}</strong>
        </span>
        <span
          :class="
            sizeDiff.diff <= 0
        ...
        >
          ...
          {{
            sizeDiff.diff <= 0
              ? t("tools.cssJsMinifier.smaller")
              : t("tools.cssJsMinifier.larger")
          }}
        </span>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          ...
          <LineNumberedTextarea
            ...
            :placeholder="
              language === 'css'
                ? t('tools.cssJsMinifier.cssPlaceholder')
                : t('tools.cssJsMinifier.jsPlaceholder')
            "
          />
        </div>
        ...
        <div class="p-6 card">
          ...
          <div
            v-if="loading"
            class="h-96 flex items-center justify-center text-gray-500 dark:text-gray-400"
          >
      ...
      <div class="p-6 mt-8 card">
        ...
        <div
          class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-3"
        >
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.cssJsMinifier.infoFormatLabel") }}
            </div>
            <p class="text-xs">{{ t("tools.cssJsMinifier.infoFormat") }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.cssJsMinifier.infoMinifyLabel") }}
            </div>
            <p class="text-xs">{{ t("tools.cssJsMinifier.infoMinify") }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">
              {{ t("tools.cssJsMinifier.infoPrivacyLabel") }}
            </div>
