...
const { t } = useI18n();
useSeoMeta({
  titleKey: "tools.jsonCompare.title",
  descriptionKey: "tools.jsonCompare.description",
});
...
function formatBoth() {
  ...
  try {
    ...
    error.value =
      ...
      t("tools.jsonCompare.formatModifiedError") +
      e.message;
  }
}
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
          <GitCompare class="inline-block w-5 h-5" />
          {{ t("tools.jsonCompare.title") }}
        </h1>
      ...
      <div class="p-6 mb-6 card">
        <div class="flex flex-wrap items-center gap-3">
          <button @click="compareJSON" class="btn-primary">
            <Search class="inline-block w-4 h-4" />
            {{ t("tools.jsonCompare.compare") }}
          </button>
          <button @click="formatBoth" class="btn-secondary">
            <Paintbrush class="inline-block w-4 h-4" />
            {{ t("tools.jsonCompare.formatBoth") }}
          </button>
          <button @click="clearAll" class="btn-secondary">
            <Trash2 class="inline-block w-4 h-4" />
            {{ t("tools.jsonCompare.clearAll") }}
          </button>
          <button @click="swapInputs" class="btn-secondary">
            <ArrowLeftRight class="inline-block w-4 h-4" />
            {{ t("tools.jsonCompare.swap") }}
          </button>
          <label
            class="flex items-center space-x-2 ml-auto text-gray-700 dark:text-gray-300"
          >
            <input
              type="checkbox"
            ...
            />
            <span class="text-sm">{{
              t("tools.jsonCompare.ignoreWhitespace")
            }}</span>
          </label>
        </div>
      </div>
      ...
      <div v-if="!compared" class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">
              {{ t("tools.jsonCompare.originalJson") }}
            </h3>
        ...
        <div class="p-6 card">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">
              {{ t("tools.jsonCompare.modifiedJson") }}
            </h3>
            <span class="text-sm text-gray-500 dark:text-gray-400">
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-4">
          <div class="p-4 card bg-green-50 dark:bg-green-900/20">
            ...
            <div class="text-sm text-green-700 dark:text-green-300">
              {{ t("tools.jsonCompare.linesAdded") }}
            </div>
          </div>
          <div class="p-4 card bg-red-50 dark:bg-red-900/20">
            ...
            <div class="text-sm text-red-700 dark:text-red-300">
              {{ t("tools.jsonCompare.linesDeleted") }}
            </div>
          </div>
          <div class="p-4 card bg-yellow-50 dark:bg-yellow-900/20">
            ...
            <div class="text-sm text-yellow-700 dark:text-yellow-300">
              {{ t("tools.jsonCompare.linesModified") }}
            </div>
          </div>
          <div class="p-4 card bg-blue-50 dark:bg-blue-900/20">
            ...
            <div class="text-sm text-blue-700 dark:text-blue-300">
              {{ t("tools.jsonCompare.totalChanges") }}
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div
            class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-night-border"
          >
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">
              {{ t("tools.jsonCompare.comparison") }}
            </h3>
          ...
          <div
            class="grid lg:grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700"
          >
            <!-- Left side (Original) -->
            <div class="overflow-x-auto">
              <div
                class="p-4 bg-gray-100 dark:bg-night-card-inner border-b border-gray-200 dark:border-night-border"
              >
                <h4
                  class="text-sm font-medium text-gray-700 dark:text-gray-300"
                >
                  {{ t("tools.jsonCompare.original") }}
                </h4>
              </div>
                ...
                <div
                  ...
                  :class="{
                    ...
                    'bg-yellow-50 dark:bg-yellow-900/20':
                      line.type === 'modify',
                    'bg-gray-100 dark:bg-night-card-inner/50':
                      line.type === 'equal',
                    'bg-transparent': line.type === 'empty',
                  }"
                >
                  <span
                    class="inline-block w-12 px-2 text-right text-gray-500 dark:text-gray-500 select-none border-r border-gray-300 dark:border-night-border flex-shrink-0"
                    :class="{
                  ...
                  <span
                    class="inline-block w-8 px-1 text-center flex-shrink-0"
                    :class="{
                  ...
                  <span
                    class="flex-1 px-2 py-1 whitespace-pre overflow-x-auto"
                    :class="{
      ...
      <div
        v-if="error"
        class="p-4 mt-6 border border-red-200 rounded-lg card bg-red-50 dark:bg-red-900/20 dark:border-red-800"
      >
