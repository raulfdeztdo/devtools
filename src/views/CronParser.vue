...
useSeoMeta({
  ...
  descriptionKey: "tools.cronParser.description",
});
...
function replaceNames(value, map) {
  return value
    .toLowerCase()
    .replace(/[a-z]+/g, (m) => (map[m] !== undefined ? String(map[m]) : m));
}
...
function validateExpression(expr) {
  ...
  if (parts.length !== 5)
    return { valid: false, error: t("tools.cronParser.error5Fields") };
  const [min, hr, dom, mon, dow] = parts;
  if (!parseField(min, 0, 59))
    return { valid: false, error: t("tools.cronParser.errorMinute") };
  if (!parseField(hr, 0, 23))
    return { valid: false, error: t("tools.cronParser.errorHour") };
  if (!parseField(dom, 1, 31))
    return { valid: false, error: t("tools.cronParser.errorDom") };
  if (!parseField(mon, 1, 12, MONTH_NAMES))
    return { valid: false, error: t("tools.cronParser.errorMonth") };
  if (!parseField(dow, 0, 7, DOW_NAMES))
    return { valid: false, error: t("tools.cronParser.errorDow") };
  return { valid: true };
}
...
function buildDescription(expr) {
  ...
  const minuteDesc = describeField(
    min,
    t("tools.cronParser.unitMinute"),
    0,
    59,
    null,
    null,
  );
  const hourDesc = describeField(
    hr,
    t("tools.cronParser.unitHour"),
    0,
    23,
    null,
    null,
  );
  const domDesc = describeField(
    dom,
    t("tools.cronParser.unitDom"),
    1,
    31,
    null,
    null,
  );
  const monDesc = describeField(
    mon,
    t("tools.cronParser.unitMonth"),
    1,
    12,
    MONTHS,
    MONTH_NAMES,
  );
  const dowDesc = describeField(
    dow,
    t("tools.cronParser.unitDow"),
    0,
    6,
    DAYS,
    DOW_NAMES,
  ...
  if (min === "*" && hr === "*") {
    ...
    desc += t("tools.cronParser.descAtTime", {
      minute: minuteDesc,
      hour: hourDesc,
    });
  }
  ...
  if (dom !== "*" && dow === "*") {
    ...
    desc +=
      " " +
      t("tools.cronParser.descOnDayOrDow", { day: domDesc, dow: dowDesc });
  }
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="mb-3 text-2xl font-bold text-gray-900 dark:text-white">
          <Clock class="inline-block w-6 h-6 mr-2" />{{
            t("tools.cronParser.title")
          }}
        </h1>
        <p class="text-gray-600 dark:text-gray-400">
          {{ t("tools.cronParser.subtitle") }}
        </p>
      </div>
      ...
      <div class="p-6 card mb-6">
        ...
        <!-- Field labels -->
        <div
          class="grid grid-cols-5 gap-2 text-xs text-center text-gray-500 dark:text-gray-400 mb-4 font-mono"
        >
        ...
        <div
          v-if="error"
          class="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-lg text-red-800 dark:text-red-300 text-sm"
        >
        ...
        <div
          ...
          class="flex items-start gap-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-700 rounded-lg text-green-800 dark:text-green-300 text-sm"
        >
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="p-6 card">
          ...
          <div class="space-y-3">
            ...
            >
              <label
                class="block mb-1 text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider"
              >
                {{
                  t(
                    "tools.cronParser.field" +
        ...
        <div class="p-6 card">
          ...
          >
            ...
            {{ t("tools.cronParser.presetsTitle") }}
          </h2>
          <div class="space-y-2">
            <button
            ...
            >
              ...
              <div
                class="text-xs font-mono text-gray-500 dark:text-gray-400 mt-0.5"
              >
                {{ preset.value }}
              </div>
            </button>
        ...
        <div class="p-6 card">
          ...
          <ol v-else class="space-y-2">
            ...
            >
              <span
                class="text-xs font-bold text-brand-blue dark:text-brand-blue-lighter w-5 text-right shrink-0"
                >{{ idx + 1 }}</span
              >
              <span
      ...
      <div class="p-6 mt-8 card">
        ...
        >
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-mono font-bold text-gray-900 dark:text-white mb-1">
              *
            </div>
