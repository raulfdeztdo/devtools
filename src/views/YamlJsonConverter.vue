...
const EXAMPLE_JSON = JSON.stringify(
    { name: 'John Doe', age: 30, active: true, hobbies: ['coding', 'reading'], address: { city: 'Madrid', country: 'Spain' } },
    null,
    2
)

const inputLabel = computed(() =>
    direction.value === 'yaml-to-json' ? t('tools.yamlJson.yamlLabel') : t('tools.yamlJson.jsonLabel')
)
const outputLabel = computed(() =>
    direction.value === 'yaml-to-json' ? t('tools.yamlJson.jsonLabel') : t('tools.yamlJson.yamlLabel')
)
const inputPlaceholder = computed(() =>
    direction.value === 'yaml-to-json' ? t('tools.yamlJson.yamlPlaceholder') : t('tools.yamlJson.jsonPlaceholder')
)
const outputPlaceholder = computed(() =>
    direction.value === 'yaml-to-json' ? t('tools.yamlJson.jsonOutputPlaceholder') : t('tools.yamlJson.yamlOutputPlaceholder')
)
...
function swapDirection() {
    ...
    error.value = ''
    direction.value = direction.value === 'yaml-to-json' ? 'json-to-yaml' : 'yaml-to-json'
        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="mb-8 text-center">
                <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
                    <ArrowLeftRight class="inline-block w-5 h-5 mr-2" />{{ t('tools.yamlJson.title') }}
                </h1>
        ...
        <div class="p-6 mt-8 card">
            ...
            <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.yamlJson.infoYamlLabel') }}</div>
                    <p class="text-xs">{{ t('tools.yamlJson.infoYaml') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.yamlJson.infoJsonLabel') }}</div>
                    <p class="text-xs">{{ t('tools.yamlJson.infoJson') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.yamlJson.infoDiffLabel') }}</div>
                    <p class="text-xs">{{ t('tools.yamlJson.infoDiff') }}</p>
                </div>
                <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
                    <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.yamlJson.infoUsageLabel') }}</div>
