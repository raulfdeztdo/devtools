...
import { ShieldCheck, CheckCircle, Trash2, Search, PartyPopper, XCircle } from 'lucide-vue-next'
...
const loadRemoteRef = async (ajv, rootSchema) => {
    ...
    const visit = (obj) => {
        if (obj && typeof obj === 'object') {
            if (obj.$ref && typeof obj.$ref === 'string' && /^https?:\/\//.test(obj.$ref)) {
                refs.add(obj.$ref)
            }
...
const validateSchema = async () => {
    ...
    try {
        ...
        const effectiveDraft = schemaDraft.value === 'auto' ? detectDraftFromSchema(parsedSchema) : schemaDraft.value
        ...
        try {
            ...
            validationResult.value = {
                isValid: false,
                errors: [{ message: t('tools.schemaValidator.invalidSchema', { message: e.message }) }]
            }
            return
        }
        ...
        validationResult.value = {
            isValid: false,
            errors: [{ message: t('tools.schemaValidator.syntaxError', { message: error.message }) }]
        }
    }
...
<template>
    <div class="py-8">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="flex flex-wrap gap-3">
                        <button @click="validateSchema" class="btn-primary">
                            <CheckCircle class="inline-block w-4 h-4 mr-1 align-text-bottom" />
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <select v-model="selectedExample" @change="loadExample" class="text-sm input-field">
                            ...
                            <option value="person">{{ t('tools.schemaValidator.person') }}</option>
                            <option value="product">{{ t('tools.schemaValidator.product') }}</option>
                            <option value="contact">{{ t('tools.schemaValidator.contact') }}</option>
                            <option value="movie">{{ t('tools.schemaValidator.movie') }}</option>
                            <option value="integer_test">{{ t('tools.schemaValidator.numberTest') }}</option>
                            <option value="api_response">{{ t('tools.schemaValidator.apiResponse') }}</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-6 mb-6 lg:grid-cols-2">
                ...
                <div class="p-6 card">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('tools.schemaValidator.jsonSchema') }}</h3>
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            {{ schema.length }} {{ t('common.characters') }}
                        </span>
                    </div>
                    <LineNumberedTextarea v-model="schema" class="textarea-field" :minHeight="'20rem'"
                        :placeholder="t('tools.schemaValidator.schemaPlaceholder')" :spellcheck="false" />
                </div>

                <!-- JSON Data -->
                <div class="p-6 card">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('tools.schemaValidator.jsonData') }}</h3>
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            {{ jsonData.length }} {{ t('common.characters') }}
                        </span>
                    </div>
                    <LineNumberedTextarea v-model="jsonData" class="textarea-field" :minHeight="'20rem'"
                        :placeholder="t('tools.schemaValidator.dataPlaceholder')" :spellcheck="false" />
                </div>
            </div>
            ...
            <div class="p-6 card">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('tools.schemaValidator.validationResult') }}</h3>
                    <div v-if="validationResult" class="flex items-center space-x-2">
                        ...
                        <span v-else
                            ...
                            {{ t('tools.schemaValidator.errorCount', { count: validationResult.errors?.length || 0 }) }}
                        </span>
                        <span class="text-xs text-gray-500 dark:text-gray-400">Draft: {{ schemaDraft }}</span>
                    </div>
                </div>

                <!-- No validation yet -->
                <div v-if="!validationResult" class="py-12 text-center text-gray-500 dark:text-gray-400">
                ...
                <div v-else class="space-y-4">
                    <div class="p-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20 dark:border-red-800">
                        <h4 class="mb-3 text-base font-semibold text-red-800 dark:text-red-200">
                            {{ t('tools.schemaValidator.errorsFound', { count: validationResult.errors.length }) }}
                        </h4>
                                ...
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div v-if="error.params"
                                            ...
                                            {{ t('tools.schemaValidator.params') }}: {{ JSON.stringify(error.params) }}
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div v-if="schemaInfo" class="p-6 mt-6 card">
                <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">{{ t('tools.schemaValidator.schemaInfo') }}</h3>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-brand-blue">{{ schemaInfo.properties }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">{{ t('tools.jsonlint.properties') }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-brand-blue">{{ schemaInfo.required }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">{{ t('tools.schemaValidator.required') }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-brand-blue">{{ schemaInfo.types }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">{{ t('tools.schemaValidator.types') }}</div>
                    </div>
                </div>
            </div>
        </div>
