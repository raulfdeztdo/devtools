...
import { Regex, AlertTriangle, ClipboardCopy, CheckCircle, Info } from 'lucide-vue-next'
...
const highlightedHtml = computed(() => {
  ...
  try {
    ...
    while ((m = re.exec(testString.value)) !== null) {
      ...
      if (m[0].length === 0) { re.lastIndex++; lastIndex++ }
    }
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
          <Regex class="inline-block w-5 h-5" /> {{ t('tools.regexTester.title') }}
        </h1>
      ...
      <div class="p-6 mb-6 card">
        <div class="mb-4">
          ...
          <div class="flex gap-2 items-center">
            ...
            />
            <span class="text-xl text-gray-400 font-mono select-none">/</span>
            <span class="font-mono text-brand-blue dark:text-brand-blue-lighter">{{ flags }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-4">
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
            ...
            <span class="font-mono font-bold">g</span> — {{ t('tools.regexTester.flagGlobal') }}
          </label>
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
            <input v-model="flagI" type="checkbox" class="checkbox" />
            <span class="font-mono font-bold">i</span> — {{ t('tools.regexTester.flagIgnoreCase') }}
          </label>
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
            <input v-model="flagM" type="checkbox" class="checkbox" />
            <span class="font-mono font-bold">m</span> — {{ t('tools.regexTester.flagMultiline') }}
          </label>
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
            <input v-model="flagS" type="checkbox" class="checkbox" />
            <span class="font-mono font-bold">s</span> — {{ t('tools.regexTester.flagDotAll') }}
          </label>
        </div>
        ...
        >
          ...
          <span class="text-sm text-red-700 dark:text-red-300 font-mono">{{ regexResult.error }}</span>
        </div>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          ...
          <div v-if="!pattern" class="text-sm text-gray-400 dark:text-gray-500 italic">
            {{ t('tools.regexTester.emptyState') }}
          </div>
          <div v-else-if="matches.length === 0 && !regexResult.error && testString" class="text-sm text-gray-400 dark:text-gray-500 italic">
          ...
          <div v-else class="space-y-2 max-h-80 overflow-y-auto">
            ...
            >
              <div class="flex items-center gap-2 mb-1">
                <span class="text-xs font-semibold text-brand-blue dark:text-brand-blue-lighter">
                  #{{ idx + 1 }}
                </span>
              ...
              <div v-if="m.groups.length" class="mt-1 space-y-0.5">
                ...
                >
                  {{ t('tools.regexTester.groupLabel') }} {{ gi + 1 }}: <span class="text-brand-orange">"{{ g ?? 'undefined' }}"</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="p-6 mt-8 card">
        ...
        <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.regexTester.infoFlagsLabel') }}</div>
            <p class="text-xs">{{ t('tools.regexTester.infoFlags') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.regexTester.infoGroupsLabel') }}</div>
            <p class="text-xs">{{ t('tools.regexTester.infoGroups') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.regexTester.infoAnchorsLabel') }}</div>
            <p class="text-xs">{{ t('tools.regexTester.infoAnchors') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.regexTester.infoQuantLabel') }}</div>
