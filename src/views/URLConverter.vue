...
function buildURL() {
  try {
    ...
    // Add path
    if (urlPath.value) {
      const encodedPath = urlPath.value
        .split("/")
...
function loadSampleURL() {
  urlInput.value =
    "https://example.com/búsqueda de productos?q=teléfono móvil&precio>100&categoría=electrónicos&disponible=sí";
  encodeURL();
}

function loadSampleEncodedURL() {
  encodedInput.value =
    "https://example.com/b%C3%BAsqueda%20de%20productos?q=tel%C3%A9fono%20m%C3%B3vil&precio%3E100&categor%C3%ADa=electr%C3%B3nicos&disponible=s%C3%AD";
  decodeURL();
}
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          ...
          <div class="space-y-4">
            ...
            <div
              v-if="encodedURL && urlInput"
              class="text-xs text-gray-500 dark:text-gray-400"
            >
              <div>
                {{ t("tools.url.originalChars") }}: {{ urlInput.length }}
              </div>
              <div>
                {{ t("tools.url.encodedChars") }}: {{ encodedURL.length }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 card">
          ...
          >
            ...
            {{ t("tools.url.decodeUrl") }}
          </h3>

          <div class="space-y-4">
            <div>
              <label
                for="encoded-url-input"
                class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300"
              >
                {{ t("tools.url.encodedUrlInput") }}
              </label>
              <textarea
                id="encoded-url-input"
            ...
            <div class="flex gap-2">
              ...
              >
                <Trash2 class="inline-block w-4 h-4" />
                {{ t("tools.url.clearDecoder") }}
              </button>
              <button
                @click="loadSampleEncodedURL"
                class="px-4 py-2 text-sm btn-secondary"
              >
                <FileText class="inline-block w-4 h-4" />
                {{ t("tools.url.sampleEncodedUrl") }}
              </button>
            </div>
            ...
            >
              <div>
                {{ t("tools.url.decodedChars") }}: {{ decodedURL.length }}
              </div>
              <div v-if="isValidURL">
                <CheckCircle class="inline-block w-4 h-4 text-green-500" />
                {{ t("tools.url.validUrl") }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="parsedParams.length > 0" class="p-6 mt-8 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          <Search class="inline-block w-5 h-5" />
          {{ t("tools.url.queryParams") }}
        </h3>
          ...
          <table class="w-full text-sm text-gray-700 dark:text-gray-300">
            <thead>
              <tr class="border-b border-gray-200 dark:border-night-border">
                <th
                  class="p-2 font-medium text-left text-gray-700 dark:text-gray-300"
                >
                  {{ t("tools.url.parameter") }}
                </th>
                <th
                  class="p-2 font-medium text-left text-gray-700 dark:text-gray-300"
                >
            ...
            <tbody>
              ...
              >
                <td class="p-2 font-mono text-gray-900 dark:text-white">
                  {{ param.key }}
                </td>
                <td
                  class="p-2 font-mono text-gray-900 break-all dark:text-white"
                >
                  {{ param.value }}
                </td>
                <td class="p-2">
                  <button
                    @click="copyToClipboard(`${param.key}=${param.value}`)"
      ...
      <div class="p-6 mt-8 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          <Wrench class="inline-block w-5 h-5" />
          {{ t("tools.url.urlBuilder") }}
        </h3>
        ...
        <div v-if="builtURL" class="mt-4">
          <div class="flex items-center justify-between mb-2">
            ...
            <button
              @click="copyToClipboard(builtURL)"
              class="p-1 text-sm btn-secondary"
              :title="t('common.copy')"
            >
              <ClipboardCopy class="inline-block w-4 h-4" />
      ...
      <div class="p-6 mt-8 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          <Info class="inline-block w-5 h-5" /> {{ t("tools.url.infoTitle") }}
        </h3>
        <div
        ...
        >
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.url.whyLabel")
            }}</span>
            {{ t("tools.url.infoWhy") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.url.specialLabel")
            }}</span>
            {{ t("tools.url.infoSpecial") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.url.componentLabel")
            }}</span>
            {{ t("tools.url.infoComponent") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.url.usageLabel")
            }}</span>
