...
const stats = computed(() => {
  ...
  diffResult.value.forEach(part => {
    const count = diffMode.value === 'lines'
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8">
        <h1 class="flex items-center gap-2 text-2xl font-bold text-gray-900 dark:text-white">
          <FileText class="w-6 h-6 text-brand-blue dark:text-brand-blue-lighter" />
          {{ t('tools.textDiff.title') }}
        </h1>
        <p class="mt-2 text-gray-600 dark:text-gray-400">{{ t('tools.textDiff.subtitle') }}</p>
      </div>
      ...
      <div class="p-4 mb-6 card flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">{{ t('tools.textDiff.mode') }}:</span>
          <button
            @click="diffMode = 'lines'"
            class="px-3 py-1.5 rounded-lg text-sm font-medium transition-colors"
            :class="diffMode === 'lines'
              ? 'bg-brand-blue text-white'
              : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'"
          >{{ t('tools.textDiff.lines') }}</button>
          <button
      ...
      <div class="grid gap-6 lg:grid-cols-2 mb-6">
        <div class="p-6 card">
          <h2 class="mb-3 text-base font-semibold text-gray-900 dark:text-white">{{ t('tools.textDiff.original') }}</h2>
        ...
        <div class="p-6 card">
          <h2 class="mb-3 text-base font-semibold text-gray-900 dark:text-white">{{ t('tools.textDiff.modified') }}</h2>
          <LineNumberedTextarea
      ...
      <div v-if="diffResult.length" class="flex flex-wrap gap-4 mb-4">
        <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-50 dark:bg-green-900/20">
          <span class="w-2 h-2 rounded-full bg-green-500 flex-shrink-0"></span>
          <span class="text-sm font-medium text-green-700 dark:text-green-300">
            +{{ stats.added }} {{ diffMode === 'lines' ? t('tools.textDiff.linesAdded') : t('tools.textDiff.wordsAdded') }}
          </span>
        </div>
        <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20">
          <span class="w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></span>
          <span class="text-sm font-medium text-red-700 dark:text-red-300">
            -{{ stats.removed }} {{ diffMode === 'lines' ? t('tools.textDiff.linesRemoved') : t('tools.textDiff.wordsRemoved') }}
          </span>
        </div>
      </div>
      ...
      <div v-if="diffResult.length" class="card overflow-hidden">
        <div class="p-3 bg-gray-100 dark:bg-night-card-inner border-b border-gray-200 dark:border-night-border flex items-center gap-2">
          <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ t('tools.textDiff.diffOutput') }}</span>
        </div>

        <!-- Lines mode -->
        <div v-if="diffMode === 'lines'" class="font-mono text-sm divide-y divide-gray-100 dark:divide-gray-800">
          <template v-for="(part, i) in diffResult" :key="i">
            ...
            >
              <span
                ...
                :class="{
                  'bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400': part.added,
                  'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400': part.removed,
                  'text-gray-300 dark:text-gray-600': !part.added && !part.removed,
                }"
              >
        ...
        <!-- Words mode -->
        <div v-else class="p-4 font-mono text-sm leading-7 text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words">
          <template v-for="(seg, i) in wordDiffSegments(diffResult)" :key="i">
            <span
              :class="{
                'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200 rounded px-0.5': seg.added,
                'bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-200 rounded px-0.5 line-through': seg.removed,
              }"
            >{{ seg.text }}</span>
          </template>
        </div>
      ...
      <div v-if="!originalText && !modifiedText" class="p-12 text-center card">
        ...
        <p class="text-gray-500 dark:text-gray-400">{{ t('tools.textDiff.emptyState') }}</p>
      </div>
      ...
      <div class="p-6 mt-8 card">
        ...
        <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.textDiff.infoLinesLabel') }}</div>
            <p class="text-xs">{{ t('tools.textDiff.infoLines') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.textDiff.infoWordsLabel') }}</div>
            <p class="text-xs">{{ t('tools.textDiff.infoWords') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.textDiff.infoDiffLabel') }}</div>
            <p class="text-xs">{{ t('tools.textDiff.infoDiff') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.textDiff.infoUsageLabel') }}</div>
