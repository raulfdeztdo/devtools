    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          ...
          <div class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                <span v-if="errorLevel === 'L'">{{ t('tools.qrGenerator.levelLDesc') }}</span>
                <span v-else-if="errorLevel === 'M'">{{ t('tools.qrGenerator.levelMDesc') }}</span>
                <span v-else-if="errorLevel === 'Q'">{{ t('tools.qrGenerator.levelQDesc') }}</span>
          ...
          <!-- Security notice -->
          <div class="flex items-start gap-2 p-3 mt-5 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
            <ShieldCheck class="w-4 h-4 mt-0.5 flex-shrink-0 text-green-600 dark:text-green-400" />
        ...
        <div class="p-6 card flex flex-col items-center">
          <h2 class="mb-4 w-full text-base font-semibold text-gray-900 dark:text-white">
            {{ t('tools.qrGenerator.previewTitle') }}
          </h2>

          <!-- Error -->
          <div v-if="error" class="w-full p-3 mb-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
          ...
          <!-- Canvas -->
          <div class="flex items-center justify-center p-4 rounded-xl bg-white dark:bg-white border border-gray-200 dark:border-night-border w-full">
          ...
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ size }}×{{ size }}px · {{ t('tools.qrGenerator.errorLevelLabel') }}: {{ errorLevel }}
          </p>
        </div>
      ...
      <div class="p-6 mt-8 card">
        ...
        <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.qrGenerator.infoErrorLabel') }}</div>
            <p class="text-xs">{{ t('tools.qrGenerator.infoError') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.qrGenerator.infoUsageLabel') }}</div>
            <p class="text-xs">{{ t('tools.qrGenerator.infoUsage') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.qrGenerator.infoSizeLabel') }}</div>
            <p class="text-xs">{{ t('tools.qrGenerator.infoSize') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.qrGenerator.infoPrivacyLabel') }}</div>
