...
const { t } = useI18n();
useSeoMeta({
  titleKey: "tools.phpSerializer.title",
  descriptionKey: "tools.phpSerializer.description",
});
...
function jsonToPhpSerialize(value) {
  ...
  throw new Error(
    t("tools.phpSerializer.unsupportedType", { type: typeof value }),
  );
}
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="mb-8 text-center">
        <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
          <Braces class="inline-block w-5 h-5" />
          {{ t("tools.phpSerializer.title") }}
        </h1>
      ...
      <div class="p-6 mb-6 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          <ArrowDownToLine class="inline-block w-5 h-5" />
          {{ t("tools.phpSerializer.jsonToPhp") }}
        </h3>
        <div class="grid gap-6 lg:grid-cols-2">
          <div>
            ...
            <textarea
              ...
              @input="convertJsonToPhp"
            ></textarea>
            <div
              v-if="jsonError"
              class="flex items-center gap-2 mt-2 text-red-800 dark:text-red-200"
            >
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <button
                v-if="phpResult"
                @click="copyToClipboard(phpResult)"
                class="px-3 py-1 text-xs btn-secondary"
              >
                <ClipboardCopy class="inline-block w-4 h-4 mr-1" />
                {{ t("common.copy") }}
              </button>
            </div>
            <textarea
              ...
              rows="10"
              readonly
              :placeholder="t('tools.phpSerializer.phpResultPlaceholder')"
            ></textarea>
            <div
              v-if="jsonInput && phpResult"
              class="mt-2 text-xs text-gray-500 dark:text-gray-400"
            >
              {{ t("common.size") }}: {{ phpResult.length }}
              {{ t("common.characters") }}
            </div>
          </div>
        </div>
      ...
      <div class="p-6 mb-6 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          <ArrowUpFromLine class="inline-block w-5 h-5" />
          {{ t("tools.phpSerializer.phpToJson") }}
        </h3>
        <div class="grid gap-6 lg:grid-cols-2">
          <div>
            <label
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <button
                v-if="jsonResult"
                @click="copyToClipboard(jsonResult)"
                class="px-3 py-1 text-xs btn-secondary"
              >
                <ClipboardCopy class="inline-block w-4 h-4 mr-1" />
                {{ t("common.copy") }}
            ...
            <textarea
              ...
              rows="10"
              readonly
              :placeholder="t('tools.phpSerializer.jsonResultPlaceholder')"
            ></textarea>
            <div
              v-if="phpInput && jsonResult"
              class="mt-2 text-xs text-gray-500 dark:text-gray-400"
            >
              {{ t("common.size") }}: {{ jsonResult.length }}
              {{ t("common.characters") }}
            </div>
          </div>
        </div>
      ...
      <div class="p-6 mb-6 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          ...
          {{ t("tools.phpSerializer.quickExamples") }}
        </h3>
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t("tools.phpSerializer.simpleObject") }}
            </div>
            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t("tools.phpSerializer.simpleObjectDesc") }}
            </div>
          </button>
          <button
          ...
          >
            <div class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t("tools.phpSerializer.indexedArray") }}
            </div>
            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t("tools.phpSerializer.indexedArrayDesc") }}
            </div>
          </button>
          <button
          ...
          >
            <div class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t("tools.phpSerializer.nestedObject") }}
            </div>
            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t("tools.phpSerializer.nestedObjectDesc") }}
            </div>
          </button>
        </div>
      </div>
      ...
      <div class="p-6 card">
        <h3 class="mb-4 text-base font-semibold text-gray-900 dark:text-white">
          ...
          {{ t("tools.phpSerializer.infoTitle") }}
        </h3>
        <div
        ...
        >
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.phpSerializer.whatLabel")
            }}</span>
            {{ t("tools.phpSerializer.infoWhat") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.phpSerializer.formatLabel")
            }}</span>
            {{ t("tools.phpSerializer.infoFormat") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.phpSerializer.usageLabel")
            }}</span>
            {{ t("tools.phpSerializer.infoUsage") }}
          </div>
          <div>
            <span class="font-medium text-gray-900 dark:text-white">{{
              t("tools.phpSerializer.cautionLabel")
            }}</span>
