...
import { Code2, ClipboardCopy, CheckCircle, ArrowLeftRight, Trash2, Info } from 'lucide-vue-next'
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-8 text-center">
        <h1 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
          <Code2 class="inline-block w-5 h-5" /> {{ t('tools.htmlEntityEncoder.title') }}
        </h1>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="p-6 card">
          <div class="flex items-center justify-between mb-2">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ mode === 'encode' ? t('tools.htmlEntityEncoder.inputLabelEncode') : t('tools.htmlEntityEncoder.inputLabelDecode') }}
            </label>
            <div class="flex gap-2">
              <button class="p-1 btn-secondary" :title="t('common.clear')" @click="clearAll">
          ...
          <LineNumberedTextarea
            v-model="inputText"
            minHeight="16rem"
            :placeholder="mode === 'encode' ? t('tools.htmlEntityEncoder.placeholderEncode') : t('tools.htmlEntityEncoder.placeholderDecode')"
          />
        ...
        <div class="p-6 card">
          <div class="flex items-center justify-between mb-2">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ mode === 'encode' ? t('tools.htmlEntityEncoder.outputLabelEncode') : t('tools.htmlEntityEncoder.outputLabelDecode') }}
            </label>
            <div class="flex gap-2">
              <button
      ...
      <div class="p-6 mt-6 card">
        ...
        <div class="grid gap-2 sm:grid-cols-3 md:grid-cols-5">
          ...
          >
            <span class="font-mono font-bold text-brand-orange">{{ item.char }}</span>
            <span class="text-gray-400 dark:text-gray-500 mx-2">→</span>
            <span class="font-mono text-gray-700 dark:text-gray-300">{{ item.entity }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 mt-8 card">
        ...
        <div class="grid gap-4 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-2 lg:grid-cols-4">
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.htmlEntityEncoder.infoWhyLabel') }}</div>
            <p class="text-xs">{{ t('tools.htmlEntityEncoder.infoWhy') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.htmlEntityEncoder.infoWhenLabel') }}</div>
            <p class="text-xs">{{ t('tools.htmlEntityEncoder.infoWhen') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.htmlEntityEncoder.infoCommonLabel') }}</div>
            <p class="text-xs">{{ t('tools.htmlEntityEncoder.infoCommon') }}</p>
          </div>
          <div class="p-3 rounded-lg bg-gray-50 dark:bg-night-card-inner">
            <div class="font-medium text-gray-900 dark:text-white mb-1">{{ t('tools.htmlEntityEncoder.infoNamedLabel') }}</div>
